...
namespace Scroll
{
    public class InfiniteScroll : MonoBehaviour, IBeginDragHandler, IDragHandler, IScrollHandler
    {
        ...
        private bool ReachedThreshold(Transform item)
        {
            if (scrollContent.Vertical)
            {
                float posYThreshold = transform.position.y + scrollContent.Height * 0.5f + _outOfBoundsThreshold;
                float negYThreshold = transform.position.y - scrollContent.Height * 0.5f - _outOfBoundsThreshold;
                return positiveDrag
            ...
            {
                float posXThreshold = transform.position.x + scrollContent.Width * 0.5f + _outOfBoundsThreshold;
                float negXThreshold = transform.position.x - scrollContent.Width * 0.5f - _outOfBoundsThreshold;
                return positiveDrag
